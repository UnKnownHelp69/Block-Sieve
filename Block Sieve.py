...
for k in range(maxk+1): #итерируемся по каждому блоку
    ...
    for i in range(count): #итерируемся по всем простым до корня из n
        ...
        j = max(start_idx, 2) * primes[i] - start #данные две строчки кода находят с какого положения надо начинать зачеркивать простые числа, то есть грубо говоря в блоке из [15, 16], при s = 2, мы вычеркнем именно число кратное 5 - 15. А max(start_idx, 2) обрабатывают первые итерации, чтобы простые числа, которые мы уже нашли, сами себя не вычеркнули.
