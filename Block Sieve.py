...
for k in range(maxk+1): #итерируемся по каждому блоку
    ...
    for i in range(count): #итерируемся по всем простым до корня из n
        ...
        start_idx = (start + primes[i] - 1) // primes[i] #мы находим сколько уже чисел, кратных данном мы прошли + 1(чтобы начать сразу вычеркивать). +primes[i]-1 нужно чтобы найти именно пройденные+1, если делить с остатком, то может оказаться ситуация, когда число нацело делится, а так мы не перейдем в новое число, добавляя prime-1, если делится, а если не делится мы как раз таки перейдем в это + 1.
        j = max(start_idx, 2) * primes[i] - start #обрабатывают первые итерации, чтобы простые числа, которые мы уже нашли, сами себя не вычеркнули( max(start_idx, 2)). Дальше мы домножаем количество пройденных на само простое число и вычитыаем сколько мы прошли уже в других блоках, чтобы найти положение в настоящем блоке.
